...
class ReplacementRule {
  ...
  */
  public ReplacementRule(int minAge, int maxAge, AgentFactory fac) {
    ...
    this.maxAge = maxAge;
    this.fac = fac;
    this.ageDict = new ArrayDictionary<Agent, Integer>();
  }
  ...
  */
  public boolean replaceThisOne(Agent a) {
    if (a.getLifespan() == Agent.NOLIFESPAN) {
      Random r = new Random();
      this.ageDict.put(a, minAge + r.nextInt(maxAge+1 - minAge));
    }
    else {
      if (a.getAge() > (int)(this.ageDict.get(a))) {
        this.ageDict.remove(a);
        return true;
      }
    }
    return false;
